...
def generate_ass_subtitles(audio_path: str, caption_text: str, shot_index: int) -> str:
    ...
    # FIX: Alignment changed to 2 (Bottom-Center). MarginV changed to 672 (35% up from bottom).
    # This prevents multi-line text from bouncing up and down over the image center.
    ass_content = [
        "[Script Info]",
        "ScriptType: v4.00+",
        "PlayResX: 1080",
    ...
    if not words_data:
        ...
        clean_fallback = re.sub(r'[,.—!?]', '', caption_text).strip()
        ass_content.append(f"Dialogue: 0,0:00:00.00,0:00:10.00,Default,,0,0,0,,{clean_fallback}")
        with open(ass_path, "w") as f:
            f.write("\n".join(ass_content))
        return ass_path

    for i, current_word in enumerate(words_data):
        # Anchor the very first word to 0.0s to prevent initial load delay flicker
        start_time = format_ass_time(0.0 if i == 0 else current_word['start'])
        
        # Bridge the gap: stretch end time to the EXACT start time of the next word.
        if i < len(words_data) - 1:
            end_time = format_ass_time(words_data[i+1]['start'])
        else:
            end_time = format_ass_time(current_word['end'] + 0.5)
        
        line_text = ""
        for j, w in enumerate(words_data):
            # Aggressive punctuation wipe: kill Whisper's hallucinated commas/periods
