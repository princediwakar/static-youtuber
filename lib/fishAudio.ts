...
export async function callFishAudio(
  text: string,
  referenceId: string,
  ...
  if (directorNotes) {
    body.director_notes = directorNotes;
  }
  ...
  if (!res.ok) {
    ...
    throw new Error(`Fish Audio error ${res.status}: ${errorText.slice(0, 500)}`);
  }
...
export async function generateSpeech(
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      return await callFishAudio(text, referenceId);
    } catch (err: any) {
      lastError = err;
  ...
  throw lastError ?? new Error('Fish Audio TTS generation failed after all retries');
}
