...
export const SlideshowScriptSchema = z.object({
  ...
  visual_world: z.enum(['history-cinematic', 'learn-technical', 'industrial-wealth', 'raw-rebuild', 'clinical-architectural']),
  format_template: z.enum(['RAPID_FIRE', 'SLOW_BURN', 'THE_LIST', 'DEEP_DIVE']),
  title: z.string().min(5).max(100),
  description: z.string().min(30).max(1000),
  tags: z.array(z.string()).min(5).max(12),
  voiceName: z.string().min(1),
  shots: z.array(ShotSchema).min(12).max(30),
