...
export const channelScheduler = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    if (channels.length === 0) {
      console.log(`[Scheduler] No active channels found.`);
      return { accountsTriggered: 0, accountsSkipped: 0 };
    }
    ...
    for (const channel of channels) {
      ...
      if (!isCoolingDown) {
        ...
        break; // Stop after triggering the first eligible channel
      } else {
        console.log(`[Scheduler] Skipping ${channel.account_id} — still in 16h cooldown`);
        skipped++;
      }
    }

    if (triggeredAccountId) {
      console.log(`[Scheduler] Triggered ${triggeredAccountId}.`);
    } else {
      console.log(`[Scheduler] No channels eligible (all in cooldown).`);
    }
...
export const generateLongForm = inngest.createFunction(
  {
    ...
    retries: 3,
    // 2h matches the existing shorts finish timeout and is safe for free tier.
    // Actual wall time for 60 shots is ~45-60 min.
    timeouts: { finish: '2h' },
    ...
    onFailure: async ({ error, event }) => {
      ...
      try {
        ...
        if (job?.id) {
          await db.updateJob(job.id, { status: 'failed', error_message: error.message });
          console.error(`[LongForm] Marked job ${job.id} as failed`);
        }
      } catch (dbErr: any) {
  ...
  async ({ step, event }) => {
    ...
    const [scriptResult] = await Promise.all([
      step.run('generate-long-script', async () => {
        const jobToResume = explicitJobId
          ? await db.getJob(explicitJobId)
          : await db.getIncompleteJobByType(accountId, 'long'); // never picks a shorts job
        ...
        const jobId = await db.createJob({
          ...
          script,
          status: 'script_ready',
          content_type: 'long', // critical discriminator
        });
        (event as any).data.jobId = jobId;
    ...
    // ── Step 2a: Narration (same F5-TTS, 30-60 shots) ──────────────────────────
    const { shotAudioUrls, narrationDurationMs } = await step.run('generate-narration', async () => {
      const job = await db.getJob(jobId);
      if (job?.shot_audio_urls) return { shotAudioUrls: job.shot_audio_urls, narrationDurationMs: 0 };
      ...
      const urls: string[] = [];
      for (let i = 0; i < results.length; i++) {
        const url = await uploadSlideAudio(results[i].audioBuffer, jobId, i, await getAccountCredentials(accountId));
        urls.push(url);
      }
    ...
    const narrationDurationSec = Math.ceil(narrationDurationMs / 1000) || 180;
    ...
    const [imageUrls, musicUrl] = await Promise.all([
        ...
        for (let batchStart = 0; batchStart < script.shots.length; batchStart += CONCURRENCY_LIMIT) {
          ...
          const batchResults = await Promise.all(
            batch.map(async (shot: Shot, offset: number) => {
              const globalIndex = batchStart + offset;
              if (urls[globalIndex]) return urls[globalIndex];
              // 1344×768 landscape images
              const rawImageBuffer = await generateImage(shot.visual_prompt, LONG_CF_AI_SLIDE_WIDTH, LONG_CF_AI_SLIDE_HEIGHT);
              return uploadSlideImage(rawImageBuffer, jobId, globalIndex, creds);
            })
          );
      ...
      step.run('select-music', async () => {
        ...
        const creds = await getAccountCredentials(accountId);
        const narrationText = script.shots.map((s: Shot) => s.spoken_text).join(' ');
        // Cap at 300s — musicSelector no longer enforces 60s for any format
        const { buffer } = await selectMusicTrack(
          script.title, niche, format_template as FormatTemplate,
          script.visual_world, narrationText,
          Math.min(narrationDurationSec, 300),
        );
        const url = await uploadMusicTrack(buffer, jobId, creds);
        await db.updateJob(jobId, { music_url: url });
        return url;
      ...
      step.run('generate-thumbnail', async () => {
        ...
        const thumbBuffer = await generateThumbnail(thumbText, script.thumbnailPrompt, niche, LONG_THUMBNAIL_WIDTH, LONG_THUMBNAIL_HEIGHT);
        const url = await uploadThumbnail(thumbBuffer, jobId, creds);
        await db.updateJob(jobId, { thumbnail_url: url });
        return url;
      }),
      ...
      try {
        const response = await fetch(MODAL_RENDER_URL, {
          ...
          body: JSON.stringify({
            jobId,
            accountId,
            content_type: 'long',             // triggers get_render_config('long') in render.py
            visual_world: script.visual_world,
            caption_style: getCaptionStyle(script.visual_world),
            shots: script.shots.map((shot: Shot, i: number) => ({
              image_url: imageUrls[i],
        ...
        console.log(`[LongForm] Modal queued render, awaiting webhook`);
      } catch (e: any) {
        clearTimeout(timeout);
        throw e;
      }
    ...
    if (useModal && !videoUrl) {
      // Long-form renders take longer — 30m vs 10m for shorts
      const modalResult = await step.waitForEvent('wait-for-modal', {
        event: 'modal/render.complete',
        timeout: '30m',
        if: `async.data.jobId == '${jobId}'`,
      ...
      if (!modalResult?.data?.mp4Url) {
        throw new Error('Modal render did not complete within 30 minutes — webhook never arrived');
      }
      resolvedVideoUrl = modalResult.data.mp4Url;
    }
    ...
    if (!skipPublish) {
      await step.run('publish', async () => {
        ...
        const thumbBuffer = Buffer.from(await thumbRes.arrayBuffer());
        // uploadToYouTube builds description from script.tags as hashtags.
        // It does NOT add #Shorts — YouTube detects format from dimensions + duration > 60s.
        const labelAsSynthetic = Math.random() < 0.5;
        const result = await uploadToYouTube(resolvedVideoUrl, thumbBuffer, script, creds, labelAsSynthetic);
...
export const longFormScheduler = inngest.createFunction(
  {
    ...
    retries: 1,
    // 11 UTC daily — before the shorts window to avoid resource contention.
    // This is an Inngest-native cron: does NOT count against Vercel's 1-cron Hobby limit.
    triggers: [{ cron: '0 11 * * *' }],
    onFailure: async ({ error }) => {
  ...
  async ({ step }) => {
    ...
    for (const accountId of accounts) {
      ...
      if (shouldRun) {
        ...
        console.log(`[LongFormScheduler] Triggered for ${accountId}`);
        triggered++;
      } else {
